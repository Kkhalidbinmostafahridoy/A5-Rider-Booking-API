...
const requestRide = async (req, res, next) => {
    try {
        ...
        const activeRide = await rider_model_1.Ride.findOne({
            riderId,
            // for build error
            // status: {
            //   $in: [
        ...
        const result = await rider_model_1.Ride.create(newRide);
...
//   }
// );
exports.getRideHistory = (0, catchAsync_1.catchAsync)(async (req, res) => {
    const riderId = req.user?.userId;
    if (!riderId) {
    ...
    const rides = await rider_model_1.Ride.find({ riderId }).sort({ createdAt: -1 });
    // classify
